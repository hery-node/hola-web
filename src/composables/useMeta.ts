...
export function useMeta(options: UseMetaOptions): UseMetaReturn {
  const { entity, entityLabel, fields = [], mergeWithServer = false, headers = [] } = options;
  ...
  const getEntityLabel = (): string | undefined => (entityLabel ? (typeof entityLabel === "string" ? entityLabel : entityLabel.value) : undefined);
   ...
   * mergeWithServer=true: iterate server fields, server properties override custom
   * mergeWithServer=false: iterate custom fields, custom properties override server
   */
  const getMetaFields = (customFields: EntityField[], serverFields: EntityField[]): EntityField[] => {
    if (customFields?.length > 0) {
      if (mergeWithServer) {
        // Server priority: loop server_fields, merge with custom if found
        // Original: { ...found, ...field } where found=custom, field=server
        // Server properties override custom properties
        return serverFields.map((field) => {
          const found = customFields.find((f) => f.name === field.name);
          return found ? { ...found, ...field } : { ...field };
        });
      } else {
        // Custom priority: loop custom_fields, merge with server if found
        // Original: { ...field, ...found } where field=custom, found=server
  ...
  const getFieldType = (field: EntityField): TypeDefinition => {
    ...
    if (!type) {
      throw new Error(`No type found for [${field.type}] in field: ${field.name} of entity: ${getEntity()}`);
    }
    if (!type.inputType) {
      throw new Error(`No inputType defined for [${field.type}] in field: ${field.name} of entity: ${getEntity()}`);
    }
    return type;
  };
  ...
  return { meta, entityLabel: entityLabelText, entityLabelText, loadMeta, getSearchFields, getEditFields, getCloneFields, getFormFields, getPropertyFields, getTableHeaders, getFieldType, formatFieldValue, getFieldLabel, getFieldLabelByName };
}
